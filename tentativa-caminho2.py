...
def main():
    ...
    print(f'XM: {XM}')
    MG = mp.interpolate(k_interp, [mp.Vector3(0.5, 0.5), mp.Vector3(0, 0.5)])
    print(f'XM: {XM}')
    GT = mp.interpolate(k_interp, [mp.Vector3(0, 0.5), mp.Vector3()])
    print(f'MG: {GT}')
    ...
    GT_freqs = sim.run_k_points(300, GT)
    ...
    GT_band_data = prepare_band_data(GT_freqs)
    ...
    for band in range(GX_band_data.shape[1]):
        plt.plot(GX_values, GX_band_data[:, band], 'ko', markersize=1, linewidth=0.2, label='' if band == 0 else "")

    XM_values = [k.x+0.5 for k in GX]
    print('K_y:', XM_values)
    for band in range(XM_band_data.shape[1]):
        plt.plot(XM_values, XM_band_data[:, band], 'ko', markersize=1, linewidth=0.2, label='' if band == 0 else "")

    MG_values = [k.x+1 for k in GX]
    print('K_xy:', MG_values)
    for band in range(MG_band_data.shape[1]):
        plt.plot(MG_values, MG_band_data[:, band], 'ko', markersize=1, linewidth=0.2, label='' if band == 0 else "")

    GT_values = [k.x + 1.5 for k in GX]
    print('K_xy:', GT_values)
    for band in range(GT_band_data.shape[1]):
        plt.plot(GT_values, GT_band_data[:, band], 'ko', markersize=1, linewidth=0.2, label='' if band == 0 else "")
